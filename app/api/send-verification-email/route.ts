...
export async function POST(req: NextRequest) {
  try {
    ...
    const { error } = await resend.emails.send({
      ...
      to: email,
      subject: "✅ ยืนยันอีเมลของคุณ — หอพักหยาหยี๋",
      html: `
<!DOCTYPE html>
<html lang="th">
<head>
        ...
        <table width="100%" style="max-width:520px;background:#fff;border-radius:24px;overflow:hidden;box-shadow:0 4px 32px rgba(0,0,0,0.08);">
          ...
          <tr>
            <td style="padding:36px 32px 28px;">
              <p style="margin:0 0 8px;color:#4a3728;font-size:15px;font-weight:600;">สวัสดี ${name || "คุณ"} 👋</p>
              <p style="margin:0 0 24px;color:#6b5744;font-size:14px;line-height:1.7;">
                ขอบคุณที่สมัครสมาชิกกับหอพักหยาหยี๋ออนไลน์<br/>
                กรุณากดปุ่มด้านล่างเพื่อยืนยันอีเมลของคุณ และเริ่มต้นใช้งานระบบ
              </p>
              ...
              <div style="margin-top:12px;padding:12px 16px;background:#faf6f0;border-radius:10px;border:1px solid #e8d5b7;word-break:break-all;">
                <a href="${verificationLink}" style="color:#c8a45e;font-size:11px;text-decoration:none;">${verificationLink}</a>
              </div>
            </td>
          </tr>
          ...
          <tr>
            <td style="padding:20px 32px 28px;text-align:center;">
              <p style="margin:0 0 4px;color:#b8a090;font-size:12px;">
                หากคุณไม่ได้สมัครสมาชิก สามารถเพิกเฉยต่ออีเมลนี้ได้
              </p>
